...
RefPtr<USBTransfer> USBTransfer::create(const USBPipe& pipe, u16 len)
{
    auto vmobj = ContiguousVMObject::create_with_size(PAGE_SIZE);
    if (!vmobj)
        return nullptr;

    return adopt_ref_if_nonnull(new USBTransfer(pipe, len, vmobj));
}

USBTransfer::USBTransfer(const USBPipe& pipe, u16 len, RefPtr<ContiguousVMObject>& vmobj)
    : m_pipe(pipe)
    , m_transfer_data_size(len)
{
    ...
    m_data_buffer = MemoryManager::the().allocate_kernel_region_with_vmobject(*vmobj, PAGE_SIZE, "USB Transfer Buffer", Region::Access::Read | Region::Access::Write);
}
...
void USBTransfer::set_setup_packet(const USBRequestData& request)
{
