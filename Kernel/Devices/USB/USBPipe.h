...
// to is the physical buffer created when a USBTransfer is allocated.
//
class USBPipe {
public:
    enum class Type : u8 {
...
public:
    static OwnPtr<USBPipe> create_pipe(Type type, Direction direction, u8 endpoint_address, u16 max_packet_size, i8 device_address, u8 poll_interval = 0);
    ...
    size_t control_transfer(u8 request_type, u8 request, u16 value, u16 index, u16 length, void* data);

    USBPipe(Type type, Direction direction, u16 max_packet_size);
    USBPipe(Type type, Direction direction, USBEndpointDescriptor& endpoint);
    USBPipe(Type type, Direction direction, u8 endpoint_address, u16 max_packet_size, u8 poll_interval, i8 device_address);
...
private:
    ...
    i8 m_device_address;   // Device address of this pipe
    ...
    bool m_data_toggle;    // Data toggle for stuffing bit
};
